...
require_env_key() {
  ...
  if [[ -z "${!env_key:-}" ]]; then
    ...
    log_error "Set it in OpenClaw global env vars before running /video_searching."
    exit 2
  fi
}
...
resolve_runtime_root() {
  if [[ -n "${VIDEO_SEARCHING_AGENT_ROOT:-}" ]]; then
    if [[ ! -d "${VIDEO_SEARCHING_AGENT_ROOT}" ]]; then
      log_error "VIDEO_SEARCHING_AGENT_ROOT does not point to a directory: ${VIDEO_SEARCHING_AGENT_ROOT}"
      exit 2
    fi
    echo "${VIDEO_SEARCHING_AGENT_ROOT}"
    return 0
  fi
...
main() {
  ...
  uv run python -m video_searching_agent.integrations.openclaw_runner "$@"
}
